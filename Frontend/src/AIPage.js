...
import { collection, addDoc, getDocs, orderBy, query, deleteDoc, doc, updateDoc, getDoc } from "firebase/firestore";
...
export default function ChatbotPage() {
  ...
  return (
    <div className={`${isFullScreen ? 'fixed inset-0 z-50 bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50' : 'min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-indigo-50 to-purple-50'} text-slate-800`}>
      {/* Always show navbar, but adjust styling for full-screen */}
      <div className={`${isFullScreen ? 'absolute top-0 left-0 right-0 z-[60]' : ''}`}>
        <Navbar />
      </div>
      
      <main className={`flex-1 flex flex-col w-full items-center justify-center px-4 py-8 ${isFullScreen ? 'pt-32' : 'pt-24'}`}>
        <motion.div 
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.6 }}
          className={`flex flex-col md:flex-row w-full h-full ${isFullScreen ? 'max-w-none' : 'max-w-7xl'} grow gap-8`}
        >
          ...
          <motion.div 
            ...
            className={`flex flex-col items-stretch justify-end ${isFullScreen ? 'flex-1 w-full h-[calc(100vh-120px)]' : 'w-full md:w-[500px] h-[600px]'}`}
          >
            <ChatBox 
              user={user} 
              lang={lang} 
...
function ChatBox({ user, lang, setLang, isFullScreen, setIsFullScreen }) {
  ...
  useEffect(() => {
    async function loadHistory() {
      if (user && user.uid) {
        try {
          const q = query(collection(db, "users", user.uid, "chatHistory"), orderBy("timestamp", "desc"));
  ...
  const startNewChat = () => {
    ...
    // Reset for new chat
    setMessages([{ role: "assistant", text: "Hi! I'm SomaAI. What's your question?" }]);
    setSessionId(null);
  ...
  const saveCurrentChat = async () => {
    if (messages.length > 1 && user && user.uid) {
      try {
        const chatData = {
          messages: messages,
          timestamp: new Date(),
        ...
        if (currentChatId) {
          ...
          await updateDoc(doc(db, "users", user.uid, "chatHistory", currentChatId), chatData);
        } else {
          // Create new chat
          const docRef = await addDoc(collection(db, "users", user.uid, "chatHistory"), chatData);
  ...
  const loadChat = async (chatId) => {
    try {
      const chatDoc = await getDoc(doc(db, "users", user.uid, "chatHistory", chatId));
      if (chatDoc.exists()) {
    ...
    <div className="flex w-full h-full">
      ...
      >
        ...
        <div
          ...
          style={{ 
            maxHeight: isFullScreen ? 'calc(100vh - 280px)' : 'calc(600px - 200px)',
